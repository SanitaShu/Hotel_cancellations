...
# 2) Donut chart – labels outside, no extra blue ring
if {"hotel", "is_canceled"}.issubset(df_f.columns) and len(df_f):
    ...
    # Labels OUTSIDE donut (no extra ring)
    text = (
    ...
    pie_final = (
        ...
        height=350,
        title="Cancellation Share by Hotel Type",
    )

    st.altair_chart(pie_final, use_container_width=True)


# 3) Lead bucket line chart – always show all buckets in logical order
if {"lead_bucket", "is_canceled"}.issubset(df_f.columns) and len(df_f):
    ...
    # Compute cancellation rate, then reindex so all buckets appear
    series = df_f.groupby("lead_bucket")["is_canceled"].mean()
    lead_grp = series.reindex(lead_order).reset_index()
    lead_grp.columns = ["lead_bucket", "cancel_rate"]

    # If some buckets have no rows after filtering, they become NaN → set to 0
    lead_grp["cancel_rate"] = lead_grp["cancel_rate"].fillna(0.0)
