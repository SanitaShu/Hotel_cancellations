...
# ------------------------------------------------------------
# RECOMPUTE CLEAN LEAD BUCKETS FROM LEAD_TIME
# ------------------------------------------------------------
if "lead_time" in df.columns:
    bins = [-1, 7, 30, 90, 180, 365, np.inf]
    labels = ["<=7d", "8-30d", "31-90d", "91-180d", "181-365d", ">365d"]
    df["lead_bucket_clean"] = pd.cut(df["lead_time"], bins=bins, labels=labels)
else:
    df["lead_bucket_clean"] = pd.NA
...
# 3) Lead bucket line chart – uses CLEAN buckets from lead_time
if {"lead_bucket_clean", "is_canceled"}.issubset(df_f.columns) and len(df_f):

    lead_order = ["<=7d", "8-30d", "31-90d", "91-180d", "181-365d", ">365d"]
    order_map = {b: i for i, b in enumerate(lead_order)}

    lead_grp = (
        df_f.groupby("lead_bucket_clean")["is_canceled"]
        .agg(cancel_rate="mean", count="size")
        .reset_index()
        .rename(columns={"lead_bucket_clean": "lead_bucket"})
    )
