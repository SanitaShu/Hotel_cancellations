...
def load_data():
    """
    Load hotel bookings data. Tries several likely file names (CSV/Parquet).
    """
    candidates = [
    ...
    # Drop rows with missing numeric basics
    return df.dropna().reset_index(drop=True)


# ------------------------------------------------------------
# MODEL LOADING (with sklearn compatibility patch)
# ------------------------------------------------------------
@st.cache_resource
def load_model():
    # Patch for old ColumnTransformer remainder helper
    try:
...
# ------------------------------------------------------------
# DEFAULT VALUES FOR FEATURES (used in prediction form)
# ------------------------------------------------------------
...
with st.form("prediction_form"):
    ...
    with colA:
        ...
        if "country" in df.columns and "country" in REQUIRED:
            ...
            country_label_choice = st.selectbox(
                "Country", labels, index=labels.index(default_label)
            )
            user_vals["country"] = label_to_code.get(
                country_label_choice, default_code
            )

        # Arrival month (numeric 1–12 in model; show names to user)
...
# --- Helper: numeric month → month name & number ---
...
# 1) ADR + Cancellation Rate per Month
...
# 2) Donut – Cancellation share by hotel
if {"hotel", "is_canceled"}.issubset(df_f.columns) and len(df_f):
    ...
    hotel_grp["label"] = (hotel_grp["cancel_rate"] * 100).round(1).astype(str) + "%"
    ...
    labels = (
        base.mark_text(radius=105, size=14)
        .encode(
            theta=alt.Theta("cancel_rate:Q", stack=True),
            text="label:N",
            color=alt.Color("hotel:N", legend=None),
        )
    )
