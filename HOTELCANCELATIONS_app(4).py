...
def month_name_and_number(series: pd.Series):
    """
    Given 'arrival_date_month' which may be numeric or string,
    ...
    if len(tmp):
        ...
        ch1 = (
            ...
            .encode(
                ...
                y=alt.Y("cancel_rate:Q", title="Cancellation rate", axis=alt.Axis(format=".0%")),
                tooltip=["month_name", alt.Tooltip("cancel_rate:Q", format=".1%")],
            )
            .properties(height=260)
        )

        # ADR by month with labels
        bars = (
            alt.Chart(grp)
            .mark_bar()
            .encode(
                ...
                tooltip=["month_name", alt.Tooltip("mean_adr:Q", format=".1f")],
            )
        )
        text = (
            alt.Chart(grp)
            ...
            .encode(
                x="month_name:N",
                y="mean_adr:Q",
                text=alt.Text("mean_adr:Q", format=".0f"),
            )
        )
        ch2 = (bars + text).properties(height=260)

        charts.append(("Cancellation rate by month", ch1))
        charts.append(("Average ADR by month", ch2))

# ---- ADR by hotel with labels ----
if {"hotel", "adr"}.issubset(df_f.columns) and len(df_f):
    ...
    bars2 = (
        ...
        .encode(
            ...
            tooltip=["hotel", alt.Tooltip("adr:Q", format=".1f")],
        )
    )
    text2 = (
        alt.Chart(grp2)
    ...
    ch3 = (bars2 + text2).properties(height=260)
    charts.append(("Average ADR by hotel", ch3))

# ---- Cancellation rate by deposit type ----
if {"deposit_type", "is_canceled"}.issubset(df_f.columns) and len(df_f):
    grp4 = (
        df_f.groupby("deposit_type")["is_canceled"]
        .mean()
        .reset_index()
    ...
    ch4 = (
        ...
        .mark_bar()
        .encode(
            x=alt.X("deposit_type:N", title="Deposit type"),
            y=alt.Y("is_canceled:Q", title="Cancellation rate", axis=alt.Axis(format=".0%")),
            tooltip=["deposit_type", alt.Tooltip("is_canceled:Q", format=".1%")],
        )
        .properties(height=260)
    )
    charts.append(("Cancellation rate by deposit type", ch4))
...
if charts:
    ...
    for i in range(0, len(charts), 3):
        ...
        for col, (title, ch) in zip(cols, row):
            with col:
                st.markdown(f"### {title}")
                st.altair_chart(ch, use_container_width=True)
else:
    st.info("Not enough columns to draw charts.")
