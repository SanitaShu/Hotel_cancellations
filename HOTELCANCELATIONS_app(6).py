...
line = (
    ...
    .encode(
        x=alt.X(
            "month_name:N",
            sort=alt.SortField(field="month_num", order="ascending"),  # 👈 same
        ),
        y=alt.Y(
            "cancel_rate:Q",
...
if {"hotel", "is_canceled"}.issubset(df_f.columns) and len(df_f):
    ...
    # Text labels (e.g. "27.3%")
    hotel_grp["label"] = (
        (hotel_grp["cancel_rate"] * 100).round(1).astype(str) + "%"
    )
    ...
    # Labels on the donut itself
    labels = (
        base
        .mark_text(radius=105, size=14)
        .encode(
            theta=alt.Theta("cancel_rate:Q", stack=True),
            text="label:N",
            color=alt.Color("hotel:N", legend=None),  # match slice colour
        )
    )

    donut_with_labels = (donut + labels).properties(
        height=300,
...
if {"lead_bucket", "is_canceled"}.issubset(df_f.columns) and len(df_f):
    ...
    # Define the logical order of buckets
    ...
    lead_line = (
        ...
        .encode(
            x=alt.X(
                ...
                sort=lead_order,  # <- enforce logical order
            ),
            y=alt.Y(
                "is_canceled:Q",
