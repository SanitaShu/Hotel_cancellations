...
# 2) Donut chart – improved labels & leader lines
if {"hotel", "is_canceled"}.issubset(df_f.columns) and len(df_f):
    ...
    hotel_grp["cancel_pct"] = hotel_grp["cancel_rate"] * 100
    hotel_grp["label"] = (
        hotel_grp["hotel"] + " – " + hotel_grp["cancel_pct"].round(1).astype(str) + "%"
    )
    ...
    donut = (
        ...
        .encode(
            ...
            tooltip=[
                "hotel",
                alt.Tooltip("cancel_pct:Q", title="Cancellation rate (%)", format=".1f"),
            ],
        )
    )

    # grey arcs acting as leader lines
    lines = (
        base.mark_arc(innerRadius=112, outerRadius=130, stroke="gray", strokeWidth=1)
        .encode(
            theta=alt.Theta("cancel_pct:Q", stack=True),
        )
    )
    ...
    text = (
        base.mark_text(radius=145, size=14, fontWeight="bold")
        .encode(
            theta=alt.Theta("cancel_pct:Q", stack=True),
            text="label:N",
    ...
    pie_final = (
        donut + lines + text
    ).properties(height=350, title="Cancellation Share by Hotel Type")

    st.altair_chart(pie_final, use_container_width=True)

# 3) Lead bucket line chart – strict logical order using numeric key
if {"lead_bucket", "is_canceled"}.issubset(df_f.columns) and len(df_f):

    # Map each bucket to an order index
    order_map = {
        "<=7d": 1,
        "8-30d": 2,
        "31-90d": 3,
    ...
    lead_line = (
        ...
        .encode(
            ...
            tooltip=[
                "lead_bucket",
                alt.Tooltip("is_canceled:Q", format=".1%", title="Cancellation rate"),
            ],
        )
