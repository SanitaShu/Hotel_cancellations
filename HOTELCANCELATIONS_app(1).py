...
# Build safe defaults for all required features
defaults: dict = {}
for feat in REQUIRED:
    if feat in df.columns:
        ...
        # feature not present in df at all
        defaults[feat] = 0.0
...
with st.form("manual_prediction_form"):
    ...
    # helper: categorical selectbox
    def cat_input(colname, label, column):
        if colname in REQUIRED:
            if colname in df.columns:
                opts = sorted(df[colname].dropna().astype(str).unique())
            else:
                opts = [str(defaults[colname])] if defaults[colname] != "" else []
            if not opts:
                opts = [""]
            default_val = str(defaults[colname])
            if default_val not in opts:
                default_val = opts[0]
            idx = opts.index(default_val) if default_val in opts else 0
            user_vals[colname] = column.selectbox(label, opts, index=idx)

    # helper: numeric number_input
    def num_input(colname, label, column, minv=0.0, step=1.0):
        if colname in REQUIRED:
            val = float(defaults.get(colname, 0.0))
            if colname in df.columns and pd.api.types.is_numeric_dtype(df[colname]):
                maxv = float(df[colname].max())
            else:
                maxv = max(val * 5 + 10, minv + 10)
            user_vals[colname] = column.number_input(
                label, min_value=float(minv), max_value=float(maxv), value=float(val), step=step
            )

    # Column 1: hotel, country, month
    ...
    num_input("total_of_special_requests", "Total special requests", cols2[2], minv=0.0, step=1.0)
...
st.caption(
    "Model: RandomForest pipeline loaded from rf_booking_pipeline_2025-12-11_16-53-17.joblib (or fallback)."
)
